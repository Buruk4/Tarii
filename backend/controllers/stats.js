...
exports.getStats = asyncHandler(async (req, res, next) => {
  // get all trades for this user and include their dailyMarking
  const trades = await Trade.find({ user: req.user.id }).populate(
    "dailyMarking",
  );
  ...
  // session performance from parent DailyMarking
  const sessionPerformance = trades.reduce((acc, trade) => {
    const session = trade.dailyMarking.session;
